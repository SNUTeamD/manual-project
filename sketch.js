...
let case1texts = [
  ...
  { speaker: "PLAYER", text: "오늘부터 나는 이 제약회사에서 인간의 감염병을 치료하는\n 백신을 중점적으로 연구하는 프로젝트에 참여하게 되었다." },
  { speaker: "PLAYER", text: "신입이니까 초반 며칠은 자잘한 서류 처리 작업을 맡겠지만,\n 프로젝트를 진행하다보면 중대한 업무도 맡게 될 거라 믿는다." },
  { speaker: "PLAYER", text: "질병의 치료라는 중대한 사명을 가지고 있는 만큼\n '절대로 매뉴얼을 따라야 한다’는 선배의 말을 반드시 명심해야겠다." },
...
let case3texts = [
  ...
  { speaker: "김철수", text: "매뉴얼을 따르지 않아 발생하는 문제는 회사에서 책임지지 않으므로,\n 업무를 수행하면서 이를 반드시 지켜주시기 바랍니다." }
...
function keyPressed() {
  ...
  if (stage >= 5 && key === 'm') {
   ...
   if (stage === 5) {
    if (key === 'm' || key === 'M') {
     if (showManual === false && currentTextIndex >=  lastTextIndex) {
        mToggleCount++;
      }
    }
  }
